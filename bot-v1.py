...
# === CONFIGURATION ===
BOT_TOKEN = "dsfdsdsvfsfvdghjhdsgfdhhgjgdsfaadc"  # Replace with your bot token
ADMIN_ID = "323434324"  # Replace with your Telegram user ID
...
def update_user_info(user):
    ...
    if str(user.id) not in users_info:
        ...
        users_info[str(user.id)]["last access"] = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime())
    users_info[str(user.id)]["bot access"] = True if str(
        user.id) in load_json("valid_users.json") else False
    ...
    for file_id in chunk:
        keyboard = [[
            InlineKeyboardButton(
                f"{Like}{videos_update[file_id]['likes']}",
                callback_data=f"like:{videos_update[file_id]['video no']}"),
            InlineKeyboardButton(
                f"{Dislike}{videos_update[file_id]['dislikes']}",
                callback_data=f"dislike:{videos_update[file_id]['video no']}")
        ]]
        ...
        if str((user.username, user.full_name,
                user.link)) not in videos_update[str(file_id)]["watched_by"]:
            videos_update[str(file_id)]["watched_by"][str(
                (user.username, user.full_name,
                 user.link))] = 1  # Add user info to watched_by
        else:
            videos_update[str(file_id)]["watched_by"][str(
                (user.username, user.full_name, user.link))] += 1
        if update.effective_user.id == ADMIN_ID:
            keyboard = [[
                InlineKeyboardButton(
                    "Delete This!",
                    callback_data=
                    f"delete_video:{videos_update[file_id]['video no']}")
            ]]
    ...
    if query.data == "send_more":
        ...
        for vid in videos_:
            if videos_[vid]["video no"] == video_no:
                keyboard = [[
                    InlineKeyboardButton(
                        f"{Like}{videos_[vid]['likes']+1}",
                        callback_data=f"like:{videos_[vid]['video no']}"),
                    InlineKeyboardButton(
                        f"{Dislike}{videos_[vid]['dislikes']}",
                        callback_data=f"dislike:{videos_[vid]['video no']}")
                ]]
                markup = InlineKeyboardMarkup(keyboard)
        ...
        for vid in videos_:
            if videos_[vid]["video no"] == video_no:
                keyboard = [[
                    InlineKeyboardButton(
                        f"{Like}{videos_[vid]['likes']}",
                        callback_data=f"like:{videos_[vid]['video no']}"),
                    InlineKeyboardButton(
                        f"{Dislike}{videos_[vid]['dislikes']+1}",
                        callback_data=f"dislike:{videos_[vid]['video no']}")
                ]]
                markup = InlineKeyboardMarkup(keyboard)
                await query.edit_message_reply_markup(reply_markup=markup)
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_id not in videos:
        ...
        videos[file_id] = {
            ...
            "file_size(MB)": round(video.file_size / (1024 * 1024),
                                   2),  # Convert to MB
        ...
        if update.effective_user.id == ADMIN_ID:
            keyboard = [[
                InlineKeyboardButton(
                    "Delete",
                    callback_data=
                    f"delete_video:{str(videos[file_id]['video no'])}")
            ]]  # Use video no as identifier
